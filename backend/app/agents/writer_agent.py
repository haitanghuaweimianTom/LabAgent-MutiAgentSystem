...
\section{4 模型的建立与求解}        % 论文核心部分
    ...
    %   (1) 问题衔接：承接"问题分析"中的宏观思路，说明本子问题在整个论文主线中的位置
    %       → "针对XX问题，我们采用XX方法建立模型，原因是……"
    %   (2) 建模依据：为什么选这个方法？与问题特点的匹配度是什么？
    %   (3) 模型构建：完整的数学推导过程
    ...
    %   (6) 结果回扣：结果回答了什么原问题？合理吗？与直觉/常识一致吗？
    %
    % ★ 子问题之间的衔接：
...
4. 摘要中不要出现复杂公式和表格
5. 表格用 booktabs 风格（\toprule, \midrule, \bottomrule）
6. 图形标题放在图形下方
7. 论文必须是可以用 xelatex 编译的完整LaTeX代码
8. 数学公式用 equation 或 align 环境
9. 能用初等方法解决就不用高等方法；能用简单方法就不用复杂方法
