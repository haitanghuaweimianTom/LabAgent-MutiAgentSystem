...
class WriterAgent(BaseAgent):
    ...
    def get_system_prompt(self, template: str = "math_modeling") -> str:
        ...
        elif template == "research_survey":
            return RESEARCH_SURVEY_WRITER_SYSTEM
        return CUMCM_WRITER_SYSTEM

    def get_template_chapters(self, template: str = "math_modeling") -> List[ChapterPlan]:
        return CHAPTER_PLANS.get(template, CUMCM_CHAPTERS)
...
请按 JSON 格式返回评分结果。"""
    ...
    def _assemble_paper(self, chapters: List[Dict[str, Any]], template: str, section_results: List) -> Dict[str, Any]:
        ...
        # 按模板选择文档类
        if template == "coursework":
            preamble = self._coursework_preamble()
        elif template == "financial_analysis":
            preamble = self._financial_preamble()
        elif template == "research_survey":
            preamble = self._research_survey_preamble()
        else:
            preamble = self._cumcm_preamble()
