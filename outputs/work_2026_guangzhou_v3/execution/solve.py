...
# 动态计算数据目录路径，避免相对路径在不同工作目录下失效
_SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
_candidates = [
    os.path.join(_SCRIPT_DIR, "problems", "2026-guangzhouUSETHIS"),
    os.path.join(_SCRIPT_DIR, "..", "problems", "2026-guangzhouUSETHIS"),
...
for _cand in _candidates:
    _abs_cand = os.path.normpath(_cand)
    if os.path.isdir(_abs_cand) and os.path.isfile(os.path.join(_abs_cand, "brent_daily.csv")):
        DATA_DIR = _abs_cand
        break
...
if DATA_DIR is None:
    curr = _SCRIPT_DIR
    for _ in range(5):
        if os.path.isfile(os.path.join(curr, "brent_daily.csv")):
            DATA_DIR = curr
            break
        curr = os.path.dirname(curr)

if DATA_DIR is None:
    raise FileNotFoundError("无法找到包含 'brent_daily.csv' 的数据目录，请检查文件路径配置。")
