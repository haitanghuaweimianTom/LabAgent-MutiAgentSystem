...
def simulate(prices, fx, act_hist, psi, lambdas):
    ...
    for i in range(step, len(prices), step):
        ...
        act.append(float(a))
        last_p = p_curr
    m = float(np.mean((np.array(act[:len(act_hist)]) - act_hist)**2))
    return theo, act, m

def main():
    data_paths = {
        "macro": "2026-guangzhouUSETHIS/macro_data.xlsx",
        "brent": "2026-guangzhouUSETHIS/brent_daily.csv",
        "hist": "2026-guangzhouUSETHIS/china_oil_adjust_history.xlsx",
        "wti": "2026-guangzhouUSETHIS/wti_daily.csv",
        "cpi": "2026-guangzhouUSETHIS/china_cpi_monthly.csv",
        "ppi": "2026-guangzhouUSETHIS/china_ppi_monthly.csv"
    }
