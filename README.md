...
> **面向 CCF-A 顶会 / 数学建模竞赛的多智能体协作论文辅助生成平台。**
> 内置 8 套论文模板（含 NeurIPS 2024、ACM SIGCONF、IEEE Conference、Springer LNCS、综述、CUMCM），
> LangGraph 编排 + ReAct 工具循环 + 实时协作讨论 + 自动迭代 + 真实代码执行 + 真实 GPU 实验 + 真实 arXiv 文献检索 + 跨方法交叉验证 + Camera-Ready 自动打包 + 断点续传 + 显存监控 + **5 大自主研究能力（NAS / 自动损失函数设计 / 跨论文研究空白识别 / 代码自动演化 / AutoML）**。
